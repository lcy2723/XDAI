...
def draw_user_involve(args):
    ...
    # 轴1
    ax = sns.lineplot(data=df1, x="times", y="satisfaction_rate", color="g", marker="v")
    ax.set_xlabel("Period (Year-Month)")
    ax.set_ylabel("Satisfaction Rate (%)")
    ax.grid(False)
    plt.legend(labels=["satisfaction rate"], frameon=False)
    # 输出2
    ax2 = ax.twinx()
    sns.lineplot(data=df2, x="times", y="#rounds", color="b", marker="o", ax=ax2)
    ax2.set_ylabel("#rounds")
    ax2.grid(False)
    # ax.set_ylabel("score")
    # plt.ylim(0.6, 1.8)
    plt.legend(labels=["#rounds"], loc='upper center', frameon=False)
    plt.savefig(save_path, bbox_inches='tight')
    plt.close('all')
    print(f"finish at {save_path}")
